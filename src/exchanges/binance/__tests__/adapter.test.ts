...
import type { BinanceBookClient } from '../book-ws-client';

function makeAdapter() {
  return new BinanceAdapter(new CostCalculator(new USDConverter()));
}

describe('BinanceAdapter — setPriceBucket', () => {
  it('updates the adapter price bucket and the underlying WS client in sync', () => {
    const adapter = makeAdapter();
    const bookWs = (adapter as unknown as { bookWs: BinanceBookClient }).bookWs;
